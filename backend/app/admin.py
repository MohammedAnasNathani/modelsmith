...
def dashboard_impact(user: dict = Depends(current_user)):
    from .database import uj
    with db() as conn:
        rows = conn.execute(
            "SELECT r.id, m.name AS model, r.benchmark, r.created_at FROM runs r "
            "JOIN models m ON m.id=r.model_id JOIN projects p ON p.id=m.project_id "
            "WHERE p.owner_id=? AND r.status='success' "
            "ORDER BY r.created_at", (user["id"],)).fetchall()
    pts = []
    for r in rows:
        bm = uj(r["benchmark"], {}) or {}
        if bm.get("size_saved_pct") is None:
            continue
        pts.append({"run_id": r["id"], "model": r["model"],
                    "size_saved_pct": bm["size_saved_pct"],
