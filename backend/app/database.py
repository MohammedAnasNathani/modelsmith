...
CREATE TABLE IF NOT EXISTS api_keys (
    ...
    token_hash   TEXT UNIQUE NOT NULL,        -- sha256 of the full secret
    created_at   REAL NOT NULL,
    last_used_at REAL,
    revoked_at   REAL
);
...
def init_db() -> None:
    with db() as conn:
        ...
        rcols = {r["name"] for r in conn.execute("PRAGMA table_info(runs)")}
        if "execution_mode" not in rcols:
            conn.execute("ALTER TABLE runs ADD COLUMN execution_mode TEXT NOT NULL DEFAULT 'platform'")
        jcols = {r["name"] for r in conn.execute("PRAGMA table_info(jobs)")}
        if "mode" not in jcols:
            conn.execute("ALTER TABLE jobs ADD COLUMN mode TEXT NOT NULL DEFAULT 'platform'")
        if "runner_token" not in jcols:
            conn.execute("ALTER TABLE jobs ADD COLUMN runner_token TEXT")

        ucols = {r["name"] for r in conn.execute("PRAGMA table_info(users)")}
        if "webhook_url" not in ucols:
            conn.execute("ALTER TABLE users ADD COLUMN webhook_url TEXT NOT NULL DEFAULT ''")
