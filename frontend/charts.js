...
const MSCharts = (() => {
  ...
  function trend(container, points, opts = {}) {
    ...
    const P = 34, R = 14, B = 26;
    const s = svg(container, W, H);
    if (!points.length) return;
    const vals = points.map(p => p.value);
    const lo = Math.min(0, ...vals), hi = Math.max(...vals) * 1.15 || 1;
    const px = i => P + (points.length === 1 ? (W - P - R) / 2 : (i / (points.length - 1)) * (W - P - R));
    const py = v => H - B - ((v - lo) / (hi - lo || 1)) * (H - P - B);
    for (let i = 0; i <= 3; i++) {
      const gy = P + i * (H - P - B) / 3;
      el("line", { x1: P, y1: gy, x2: W - R, y2: gy,
                   stroke: "rgba(255,235,190,.06)", "stroke-width": 1 }, s);
      const t = el("text", { x: P - 6, y: gy + 3, "text-anchor": "end", fill: "#776c58",
                             "font-size": 9, "font-family": "JetBrains Mono,monospace" }, s);
      t.textContent = Math.round(hi * (1 - i / 3)) + (opts.unit || "");
    }
    const line = points.map((p, i) => [px(i), py(p.value)]);
    el("polyline", { points: line.map(p => p.join(",")).join(" "), fill: "none",
                     stroke: opts.color || "#34d399", "stroke-width": 2.2,
                     "stroke-linecap": "round", "stroke-linejoin": "round" }, s);
    points.forEach((p, i) => {
      el("circle", { cx: px(i), cy: py(p.value), r: 3.4,
                     fill: opts.color || "#34d399" }, s);
      const t = el("text", { x: px(i), y: py(p.value) - 9, "text-anchor": "middle",
                             fill: "#ffb224", "font-size": 10,
                             "font-family": "JetBrains Mono,monospace", "font-weight": 600 }, s);
      t.textContent = (p.value ?? "-") + (opts.unit || "");
      const xl = el("text", { x: px(i), y: H - 8, "text-anchor": "middle",
                              fill: "#776c58", "font-size": 9,
                              "font-family": "JetBrains Mono,monospace" }, s);
  ...
  return {
 donut, hbars, radar, compareBars, gauge, archGraph, sparkline, pareto, trend, PALETTE };
})();
